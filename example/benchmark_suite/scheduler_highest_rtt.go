...
// Before going through the code, if you are unfamiliar with
// golang, please go through this short tour (~ 15 mins)
// https://tour.golang.org/

// Map is a hash map storing key and value pairs,
// declared as
// map[Key_Type]Value_Type{}

type scheduler struct {
	// Each path used by the scheduler will have
	// certain quota of maximum transmitted packets.
	quotas map[protocol.PathID]uint
}

// To understand below method signature, refer
// https://stackoverflow.com/questions/34031801/function-declaration-syntax-things-in-parenthesis-before-function-name
func (sch *scheduler) setup() {
	// Make func is similar to new operator (or malloc), unlike new,
	// it returns the actual object instead of a pointer to it
...
func (sch *scheduler) selectPathRoundRobin(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	...
	if len(s.paths) <= 1 {
		// Since there is only one path, we use InitialPathID
	...
	// Find path with least amount of filled quota.
	// When all the paths have used their quota, it
	// is reset for all of the paths.
pathLoop:
	for pathID, pth := range s.paths {
		...
		// ok variable tells whether pathID key is present
		// in sch.quotas hash map or not
...
func (sch *scheduler) selectPathHighLatency(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if len(s.paths) <= 1 {
		if !hasRetransmission && !s.paths[protocol.InitialPathID].SendingAllowed() {
			return nil
...
pathLoop:
	for pathID, pth := range s.paths {
		...
		if currentRTT != 0 && higherRTT != 0 && selectedPath != nil && currentRTT <= higherRTT {
			continue pathLoop
		}

		// Update
		higherRTT = currentRTT
		selectedPath = pth
...
func (sch *scheduler) selectPath(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	...
	return sch.selectPathHighLatency(s, hasRetransmission, hasStreamRetransmission, fromPth)
