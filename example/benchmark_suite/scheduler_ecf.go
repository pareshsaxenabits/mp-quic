...
// Finds two paths which has lowest and second lowest RTT among all available paths
// These paths may not necessarily be available for transmission
func (sch *scheduler) findTwoLowestRTTPaths(s *session, 
											hasRetransmission bool, 
											hasStreamRetransmission bool, 
											fromPth *path) 
											(*path, *path)
{
	var lowestRTTPath, secondLowestRTTPath *path
	// Initialize lowestRTT to its max value
	var lowestRTT time.Duration = ^uint(0) >> 1
	var currentRTT time.Duration
	selectedPathID := protocol.PathID(255)

	// Iterate over all the paths and find the lowest RTT path
pathLoop:
	for pathID, pth := range s.paths
	{
		// If this path is potentially failed, do not consider it for sending
		...
		// What to do when path is unprobed ?

		if (currentRTT < lowestRTT)
		{
			lowestRTT = currentRTT
			secondLowestRTTPath = lowestRTTPath
			lowestRTTPath = pth
		}
	}
...
func (sch *scheduler) selectPathECF(sesn *session, 
									hasRetransmission bool, 
									hasStreamRetransmission bool, 
									fromPth *path)
									*path 
{
	// Handle case when only one path is available
	if len(sesn.paths) <= 1
	{
		if !hasRetransmission && !sesn.paths[protocol.InitialPathID].SendingAllowed()
		{
			return nil
		}
	...
	// If lowest RTT path is available for transmission, return it
	if lowestRTTPath.SendingAllowed()
	{
